...
public interface ImageRepository extends JpaRepository<Image, Long> {
    ...
    @Modifying
    @Query(value = """
            delete from actualite.image i 
            where sec_id in (
                select i.sec_id 
                from actualite.image i 
                join actualite.section s on i.sec_id = s.sec_id 
                join actualite.actualite a on s.act_id = a.act_id  
                where a.act_id = :idActualite)""", nativeQuery = true)
    void deleteAllByIdActualite(@Param("idActualite") Long idActualite);


    @Query(value = """
            select i.id from actualite.image i 
            join actualite.section s on i.sec_id = s.sec_id 
            join actualite.actualite a on s.act_id = a.act_id  
            where a.act_id = :idActualite""", nativeQuery = true)
